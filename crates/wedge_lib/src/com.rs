...
impl<T> ComPtr<T> {
    ...
    fn as_unknown(&self) -> &IUnknown { unsafe { &*(self.as_raw() as *mut IUnknown) } }
    ...
    pub fn as_raw(&self) -> *mut T { self.0.as_ptr() }
}
...
impl<T> Debug for ComPtr<T> {
    fn fmt(&self, f: &mut Formatter) -> Result<(), FmtError> { write!(f, "{:?}", self.0) }
}
...
where
    T: Interface,
{
    fn eq(&self, other: &ComPtr<T>) -> bool { self.0 == other.0 }
}
